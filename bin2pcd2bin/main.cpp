...
void readKittiPclBinData(std::string &in_file, std::string& out_file)
{
    ...
    for (i=0; input.good() && !input.eof(); i++) {
        ...
        if(!point.x==0)
            std::cout<<"point.x="<<point.x<<" "<<"point.y="<<point.y<<" "<<"point.z="<<point.z<<" "<<"point.intensity="<<point.intensity<<std::endl;
    }
    std::cout<<std::endl;
    input.close();
...
void convertPCDtoBin(std::string &in_file, std::string& out_file)//单个文件
{
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZI>);

    if (pcl::io::loadPCDFile<pcl::PointXYZI>(in_file, *cloud) == -1) //* load null file
    {
    ...
    size_t data_size = 0;
    size_t nri = 0;
    pcl::getFields (*cloud, fields);      //x y z intensity
    ...
    for (const auto &field : fields)
    {
        ...
        fsize += fs;                //4 +4 +4 +4 = 16
        fields_sizes.push_back (fs);// 4 4 4 4 
        fields[nri++] = field;      //x y z intensity nri = 4
    }
    fields.resize (nri);            //fields:x y z intensity

    data_size = cloud->points.size () * fsize; 
    const int memsize = cloud->points.size() * sizeof(float) * nri;
    std::vector<char*> v;
    ...
    {
        v.push_back((char*)&cloud->points[i].x);
        v.push_back((char*)&cloud->points[i].y);
        v.push_back((char*)&cloud->points[i].z);
        v.push_back((char*)&cloud->points[i].intensity);

    }
    std::cout<<"v.size()= :"<<v.size()<<std::endl;
    myFile.write(v[0], memsize);
    myFile.close();
}
